"""
Quantize FaceLiVT v2-L ONNX model từ FP32 → INT8.
...
Cách chạy:
    python scripts/quantize_facelivt_l_int8.py

Output:
    models/facelivtv2_l_int8.onnx
"""
import os
import sys
import glob
...
INPUT_MODEL = MODELS_DIR / "facelivtv2_l.onnx"
# Output: model INT8
OUTPUT_MODEL = MODELS_DIR / "facelivtv2_l_int8.onnx"
# Thư mục ảnh calibration (dùng dataset có sẵn)
CALIB_DATA_DIR = BASE_DIR / "dataset_clean"
...
def collect_calibration_images(data_dir: Path, max_images: int) -> list:
    ...
    if not subdirs:
        ...
        images_per_person = max(1, max_images // max(1, len(subdirs)))
...
def main():
    print("=" * 60)
    print("🔧 QUANTIZE FaceLiVT v2-L: FP32 → INT8")
    print("=" * 60)
    ...
    if not INPUT_MODEL.exists():
        ...
        print("    Vui lòng kiểm tra lại thư mục models/")
        return
    ...
    original_name = INPUT_MODEL.name
