...
class MHSA(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        qkv_perm = qkv.permute(0, 1, 3, 2)
        
        # Replace .split() with slicing to ensure compatibility with older ONNX runtimes
        idx1 = self.split_idx[0]
        idx2 = idx1 + self.split_idx[1]
        q = qkv_perm[..., :idx1]
        k = qkv_perm[..., idx1:idx2]
        v = qkv_perm[..., idx2:]
...
class MultiHeadLinearAttention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # Replace x.chunk with explicit slicing to avoid ONNX Split num_outputs (opset 18) issue
        chunk_size = self.dim // self.n_head
        x_chunks = [x[:, i*chunk_size:(i+1)*chunk_size, :] for i in range(self.n_head)]
        
        for i in range(self.n_head):
            x_chunks[i] = self.lin[i](x_chunks[i])
