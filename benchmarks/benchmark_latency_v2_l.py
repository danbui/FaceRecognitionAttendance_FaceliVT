"""
Benchmark Latency — So sánh 3 mô hình FaceLiVT2_L với ĐẦY ĐỦ PIPELINE:
...
def enroll_gallery(images, detector, embedder, max_enroll=500):
    ...
    for p in images[:max_enroll]:
        ...
        face = align_face(img, raw)
        emb = embedder.get_embedding(face).flatten()
        n = np.linalg.norm(emb)
        if n < 1e-8: continue
        embs.append(emb / n)
...
def benchmark_models(images, detector, selector, embedders, galleries, warmup=5):
    ...
    for p in images[:warmup]:
        ...
        box, raw = detector.detect_largest_with_raw(img)
        if raw is None: continue
        aligned_w = align_face(img, raw)
        for emb in embedders: emb.get_embedding(aligned_w)
...
def print_report(r, sys_info, source):
    ...
    print(f"\n{'='*100}")
    print(f"  ⏱️  BENCHMARK FULL PIPELINE — FaceLiVT2_L Variants")
    print(f"{'='*100}")
    ...
    aln_line = f"  {'3. Alignment (ArcFace)':<30}"
    ...
    emb_line = f"  {'4. Face Embedding (FaceLiVT)':<30}"
    ...
    for n in names:
        ...
        print(f"    {'Alignment (ArcFace)':<30} mean={aln['mean']:>6.2f}ms  median={aln['med']:>6.2f}ms  p95={aln['p95']:>6.2f}ms")
        print(f"    {'Face Embedding (FaceLiVT)':<30} mean={e['mean']:>6.2f}ms  median={e['med']:>6.2f}ms  p95={e['p95']:>6.2f}ms")
...
def save_csv(r, sys_info, source, out_dir):
    ...
    csv_path = out_dir / f"latency_full_pipeline_v2_l_variants_{ts}.csv"
...
def main():
    ...
    print("=" * 80)
    print("  ⏱️  BENCHMARK FULL PIPELINE: FaceLiVT2_L VARIANTS")
    print("=" * 80)
